...
// --- Example Usage Block ---
// This demonstrates how to use the ParagraphParser.
// In a real application, you would get the paragraph element from a parsed DOCX document.
async function runParagraphParserExample() {
  console.log('--- Running ParagraphParser Example ---');

  const sampleParagraphXml = `
  ...
  try {
    ...
    const { extractXmlRootFromString } = await import('../utils');
    const paragraphElement = extractXmlRootFromString(sampleParagraphXml);
    const parser = new ParagraphParser();
    const paragraph = parser.parse(paragraphElement);
    console.log('Parsed Paragraph:', JSON.stringify(paragraph, null, 2));
  } catch (error) {
    console.error('Error during ParagraphParser example:', error);
  }
  console.log('--- Finished ParagraphParser Example ---');
}
...
if (typeof process !== 'undefined' && process.argv.includes(__filename)) {
  runParagraphParserExample().catch(error => {
    console.error('Error running example directly:', error);
    process.exit(1);
  });
}

export { runParagraphParserExample }; 