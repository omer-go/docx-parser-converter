...
export function parseRunProperties(
  ...
  if (rFontsElement) {
    ...
    const ascii = extractAttribute(rFontsElement, 'w:ascii', attributeObjectPrefix);
    const hAnsi = extractAttribute(rFontsElement, 'w:hAnsi', attributeObjectPrefix);
    const eastAsia = extractAttribute(rFontsElement, 'w:eastAsia', attributeObjectPrefix);
    const cs = extractAttribute(rFontsElement, 'w:cs', attributeObjectPrefix);
  ...
  if (szElement) {
    const val = extractAttribute(szElement, 'w:val', attributeObjectPrefix);
  ...
  if (colorElement) {
    const val = extractAttribute(colorElement, 'w:val', attributeObjectPrefix);
  ...
  if (bElement !== undefined) { // Check for presence of the element itself
    props.bold = extractBooleanAttribute(bElement, 'w:val', attributeObjectPrefix);
  }
  ...
  if (iElement !== undefined) {
    props.italic = extractBooleanAttribute(iElement, 'w:val', attributeObjectPrefix);
  }

  // Underline (w:u)
  const uElement = extractElement(rPrElement, 'w:u');
  if (uElement) {
    const val = extractAttribute(uElement, 'w:val', attributeObjectPrefix);
  ...
  if (strikeElement !== undefined) {
    props.strike = extractBooleanAttribute(strikeElement, 'w:val', attributeObjectPrefix);
  }
  ...
  if (vanishElement !== undefined) {
    props.hidden = extractBooleanAttribute(vanishElement, 'w:val', attributeObjectPrefix);
  }
  ...
  if (langElement) {
    ...
    const val = extractAttribute(langElement, 'w:val', attributeObjectPrefix);
    const eastAsiaLang = extractAttribute(langElement, 'w:eastAsia', attributeObjectPrefix);
    const bidiLang = extractAttribute(langElement, 'w:bidi', attributeObjectPrefix);
  ...
  if (highlightElement) {
    const val = extractAttribute(highlightElement, 'w:val', attributeObjectPrefix);
  ...
  if (shdElement) {
    const fill = extractAttribute(shdElement, 'w:fill', attributeObjectPrefix);
    const color = extractAttribute(shdElement, 'w:color', attributeObjectPrefix);
  ...
  if (vertAlignElement) {
      const val = extractAttribute(vertAlignElement, 'w:val', attributeObjectPrefix);
      if (val) props.vert_align = val; // e.g., "superscript", "subscript", "baseline"
  }

  const positionElement = extractElement(rPrElement, 'w:position');
  if (positionElement) {
    const val = extractAttribute(positionElement, 'w:val', attributeObjectPrefix);
  ...
  if (kernElement) {
    const val = extractAttribute(kernElement, 'w:val', attributeObjectPrefix);
  ...
  if (embossElement !== undefined) {
    props.emboss = extractBooleanAttribute(embossElement, 'w:val', attributeObjectPrefix);
  }

  // Outline (w:outline)
  const outlineElement = extractElement(rPrElement, 'w:outline');
  if (outlineElement !== undefined) {
    props.outline = extractBooleanAttribute(outlineElement, 'w:val', attributeObjectPrefix);
  }
  ...
  if (capsElement !== undefined) {
    props.all_caps = extractBooleanAttribute(capsElement, 'w:val', attributeObjectPrefix);
  }

  // SmallCaps (w:smallCaps)
  const smallCapsElement = extractElement(rPrElement, 'w:smallCaps');
  if (smallCapsElement !== undefined) {
    props.small_caps = extractBooleanAttribute(smallCapsElement, 'w:val', attributeObjectPrefix);
  }
