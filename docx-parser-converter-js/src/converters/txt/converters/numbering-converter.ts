/**
 * Numbering Converter for TXT Output - Schema-Based Implementation
 * 
 * Converts DOCX numbering/list elements to plain text using the actual
...
/**
 * Converter for DOCX numbering elements to TXT - Schema-based
 */
export class NumberingConverter {
  ...
  /**
   * Convert DOCX paragraph numbering to TXT using numbering schema
   * @param numbering - DOCX paragraph numbering
   * @param context - Conversion context (should contain numbering schema)
   * @returns TXT element representing the numbering
   */
  static convertNumbering(
    ...
    if (!numberingSchema) {
      ...
      return this.generateFallbackNumbering(numbering, context);
    }
   ...
   * @returns TXT element with simple numbering
   */
  private static generateFallbackNumbering(
    numbering: ParagraphNumbering,
    context: ConversionContext
  ): TxtElement {
  ...
  private static generateSimpleNumber(level: number): string {
    // Simple fallback numbering - use bullets at different levels
    const bullets = ['•', '◦', '▪', '▫', '‣'];
